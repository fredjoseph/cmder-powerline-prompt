...
function reset_prompt_filter()
    ...
    clink.prompt.value = string.gsub(clink.prompt.value, "{lamb}", color.set_color(color.GREEN,color.BLACK, color.BOLD)..PROMPT_END_CHAR)
end
...
function admin_prompt_filter()
    if settings.display_admin then
        ...
        if ret == 0 then
            clink.prompt.value = string.gsub(clink.prompt.value, "{admin}", color.set_color(color.YELLOW,color.BLACK, color.BOLD)..PROMPT_ADMIN_CHAR.."\x1b[0m ")
            return false;
        end
    end
...
function user_prompt_filter()
    if settings.display_user then
        ...
        local user_color = color.set_color(color.WHITE, color.BLACK)
        
        clink.prompt.value = string.gsub(clink.prompt.value, "{user}", user_color..username.."@"..host)
        return false
    end
...
function cwd_prompt_filter()
    ...
    clink.prompt.value = string.gsub(clink.prompt.value, "{cwd}", color.set_color(color.WHITE, color.BLUE)..cwd)
end
    ...
    if git_dir then
        ...
        if branch then
            ...
            if get_git_status() then
                branch_color = color.set_color(color.BLACK, color.GREEN)
            else
                branch_color = color.set_color(color.BLACK, color.YELLOW)
            end
            
            clink.prompt.value = string.gsub(clink.prompt.value, "{git}", branch_color..BRANCH_SYMBOL.." "..branch)
            return false
        end
    end
...
function env_prompt_filter()
    ...
    clink.prompt.value = string.gsub(clink.prompt.value, "{env}", color.set_color(color.GREEN,color.BLACK, color.BOLD)..original_prompt_env)
end
...
local function npm_prompt_filter()
    local function npm_substitute_builder()
        ...
        return color.set_color(color.YELLOW,color.BLACK).."("..package_name..package_version..")"
    end
...
function agnoster_filter()
    ...
    local function arrow_inserter(current_seq)
        ...
        if new_back_color ~= current_back_color then
            substitute_text = color.set_color(current_back_color, new_back_color)..ARROW_SYMBOL.." "..substitute_text
